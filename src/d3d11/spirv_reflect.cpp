  ...
  SpvReflectPrvParser* p_parser)
{
  ...
  if (p_parser->spirv_code[0] != spv::MagicNumber) {
...
static SpvReflectResult ParseNodes(SpvReflectPrvParser* p_parser)
{
  ...
  while (spirv_word_index < p_parser->spirv_word_count) {
    ...
    switch (p_node->op) {
      ...
      {
        uint32_t member_offset = (p_node->op == spv::OpMemberName) ? 1 : 0;
      ...
      case spv::Decoration(4472): //spv::OpTypeRayQueryKHR:
      {
      ...
      case spv::OpAccessChain:
      {
        ...
        if (p_access_chain->index_count > 0) {
          ...
          for (uint32_t index_index = 0; index_index < p_access_chain->index_count; ++index_index) {
            ...
            if ((p_index_value_node != NULL) && (p_index_value_node->op == spv::OpConstant)) {
              // Read index value
  ...
  size_t                 first_label_index)
{
  ...
  for (size_t i = first_label_index; i < p_parser->node_count; ++i) {
    SpvReflectPrvNode* p_node = &(p_parser->nodes[i]);
    if (p_node->op == spv::OpFunctionEnd) {
      break;
    }
    switch (p_node->op) {
  ...
  if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes)) {
    ...
    for (size_t i = 0; i < p_parser->node_count; ++i) {
      ...
      for (; i < p_parser->node_count; ++i) {
        if (p_parser->nodes[i].op == spv::OpLabel) {
          func_definition = true;
          break;
        }
        if (p_parser->nodes[i].op == spv::OpFunctionEnd) {
          break;
        }
      }
      if (!func_definition) {
...
static SpvReflectResult ParseMemberCounts(SpvReflectPrvParser* p_parser)
{
  ...
  if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes)) {
    for (size_t i = 0; i < p_parser->node_count; ++i) {
      SpvReflectPrvNode* p_node = &(p_parser->nodes[i]);
      if ((p_node->op != spv::OpMemberName) && (p_node->op != spv::OpMemberDecorate)) {
        continue;
      }
...
static SpvReflectResult ParseNames(SpvReflectPrvParser* p_parser)
{
  ...
  if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes)) {
    for (size_t i = 0; i < p_parser->node_count; ++i) {
      SpvReflectPrvNode* p_node = &(p_parser->nodes[i]);
      if ((p_node->op != spv::OpName) && (p_node->op != spv::OpMemberName)) {
        continue;
      }
...
static SpvReflectResult ParseDecorations(SpvReflectPrvParser* p_parser)
{
  for (uint32_t i = 0; i < p_parser->node_count; ++i) {
    ...
    if (((uint32_t)p_node->op != (uint32_t)spv::OpDecorate) &&
        ((uint32_t)p_node->op != (uint32_t)spv::OpMemberDecorate) &&
        ((uint32_t)p_node->op != (uint32_t)spv::OpDecorateId) &&
        ((uint32_t)p_node->op != (uint32_t)spv::OpDecorateString) &&
        ((uint32_t)p_node->op != (uint32_t)spv::OpMemberDecorateString))
    {
     continue;
    }
    ...
    if (p_node->op == spv::OpMemberDecorate) {
      member_offset = 1;
    }
      ...
      case spv::OpTypeArray: {
        ...
        if (result == SPV_REFLECT_RESULT_SUCCESS) {
          ...
          if (IsNotNull(p_length_node)) {
            if (p_length_node->op == spv::OpSpecConstant ||
                p_length_node->op == spv::OpSpecConstantOp) {
  ...
  SpvReflectShaderModule* p_module)
{
  ...
  for (size_t i = 0; i < p_parser->node_count; ++i) {
    SpvReflectPrvNode* p_node = &(p_parser->nodes[i]);
    if ((p_node->op != spv::OpVariable) ||
        ((p_node->storage_class != spv::StorageClassUniform) &&
         (p_node->storage_class != spv::StorageClassStorageBuffer) &&
         (p_node->storage_class != spv::StorageClassUniformConstant)))
    {
      continue;
    }
  ...
  for (size_t i = 0; i < p_parser->node_count; ++i) {
    ...
    SpvStorageClass pointer_storage_class = spv::StorageClassMax;
    if (p_type->op == spv::OpTypePointer) {
    ...
    if (pointer_storage_class == spv::StorageClassStorageBuffer) {
    ...
    if ((int)p_descriptor->descriptor_type == (int)INVALID_VALUE) {
      switch (p_type->type_flags & SPV_REFLECT_TYPE_FLAG_EXTERNAL_MASK) {
        ...
        case SPV_REFLECT_TYPE_FLAG_EXTERNAL_IMAGE: {
          if (p_descriptor->image.dim == spv::DimBuffer) {
          ...
          else if(p_descriptor->image.dim == spv::DimSubpassData) {
...
)
{
  ...
  if (IsNotNull(p_type->members) && (p_type->member_count > 0)) {
    ...
    if (p_type_node->op == spv::OpTypeArray) {
      while (p_type_node->op == spv::OpTypeArray) {
    ...
    else if(p_type_node->op == spv::OpTypeRuntimeArray) {
    ...
    for (uint32_t member_index = 0; member_index < p_type->member_count; ++member_index) {
      ...
      if (p_member_type->op == spv::OpTypeArray) {
...
)
{
  ...
  switch (op_type) {
    ...
    case spv::OpTypeArray: {
      ...
      while ((p_type->op == spv::OpTypeArray) && (index_index < p_access_chain->index_count)) {
...
)
{
  ...
  *p_has_built_in |= p_type_node_decorations->is_built_in;
  ...
  if ((p_var->storage_class == spv::StorageClassInput) || (p_var->storage_class == spv::StorageClassOutput)) {
...
)
{
 ...
 for (size_t i = 0; i < interface_variable_count; ++i) {
   ...
   if (p_node->storage_class == spv::StorageClassInput) {
     p_entry->input_variable_count += 1;
   }
   else if (p_node->storage_class == spv::StorageClassOutput) {
 ...
 if (p_entry->output_variable_count > 0) {
  ...
  for (size_t i = 0; i < interface_variable_count; ++i) {
    ...
    if (p_var->storage_class == spv::StorageClassInput) {
  ...
  SpvReflectShaderModule* p_module)
{
  ...
  for (size_t i = 0; entry_point_index < p_parser->entry_point_count && i < p_parser->node_count; ++i) {
    ...
    switch (p_entry_point->spirv_execution_model) {
      default: break;
      case spv::ExecutionModelVertex                 : p_entry_point->shader_stage = SPV_REFLECT_SHADER_STAGE_VERTEX_BIT; break;
      case spv::ExecutionModelTessellationControl    : p_entry_point->shader_stage = SPV_REFLECT_SHADER_STAGE_TESSELLATION_CONTROL_BIT; break;
      case spv::ExecutionModelTessellationEvaluation : p_entry_point->shader_stage = SPV_REFLECT_SHADER_STAGE_TESSELLATION_EVALUATION_BIT; break;
      case spv::ExecutionModelGeometry               : p_entry_point->shader_stage = SPV_REFLECT_SHADER_STAGE_GEOMETRY_BIT; break;
      case spv::ExecutionModelFragment               : p_entry_point->shader_stage = SPV_REFLECT_SHADER_STAGE_FRAGMENT_BIT; break;
      case spv::ExecutionModelGLCompute              : p_entry_point->shader_stage = SPV_REFLECT_SHADER_STAGE_COMPUTE_BIT; break;
      case spv::ExecutionModelTaskNV                 : p_entry_point->shader_stage = SPV_REFLECT_SHADER_STAGE_TASK_BIT_NV; break;
      case spv::ExecutionModelMeshNV                 : p_entry_point->shader_stage = SPV_REFLECT_SHADER_STAGE_MESH_BIT_NV; break;
      case spv::ExecutionModelRayGenerationNV        : p_entry_point->shader_stage = SPV_REFLECT_SHADER_STAGE_RAYGEN_BIT_KHR; break;
      case spv::ExecutionModelIntersectionNV         : p_entry_point->shader_stage = SPV_REFLECT_SHADER_STAGE_INTERSECTION_BIT_KHR; break;
      case spv::ExecutionModelAnyHitNV               : p_entry_point->shader_stage = SPV_REFLECT_SHADER_STAGE_ANY_HIT_BIT_KHR; break;
      case spv::ExecutionModelClosestHitNV           : p_entry_point->shader_stage = SPV_REFLECT_SHADER_STAGE_CLOSEST_HIT_BIT_KHR; break;
      case spv::ExecutionModelMissNV                 : p_entry_point->shader_stage = SPV_REFLECT_SHADER_STAGE_MISS_BIT_KHR; break;
      case spv::ExecutionModelCallableNV             : p_entry_point->shader_stage = SPV_REFLECT_SHADER_STAGE_CALLABLE_BIT_KHR; break;
    }
  ...
  SpvReflectShaderModule* p_module)
{
  ...
  if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes)) {
    for (size_t node_idx = 0; node_idx < p_parser->node_count; ++node_idx) {
      ...
      if (p_node->op != spv::OpExecutionMode) {
        continue;
      }
  ...
  SpvReflectShaderModule* p_module)
{
  for (size_t i = 0; i < p_parser->node_count; ++i) {
    SpvReflectPrvNode* p_node = &(p_parser->nodes[i]);
    if ((p_node->op != spv::OpVariable) || (p_node->storage_class != spv::StorageClassPushConstant)) {
      continue;
    }
